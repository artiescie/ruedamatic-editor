...
<!-- modals related to move edits -->
      ...
      <b-modal id="filesWarning" title="Sequence Files Affected Warning!" @ok="modifyFilesBeforeSave">
...
export default {
  ...
  methods: {
    ...
    createWithPolly () {
      ...
      fetch(API_ENDPOINT, {
        ...
        .then(async response => {
          ...
          const bareFilename = data.substring(data.lastIndexOf('/') + 1)
      ...
      if (this.currentItemBeforeEdit) {
        ...
        if (oldName !== newName) {
          ...
          if (this.seqFilesAffected.length) {
            this.$bvModal.show('filesWarning')
            return // BAIL pending dialog answer, THEN maybe modifyFilesBeforeSave()
          }
          // if they change move length: PREVENT with warning
          //  if there are SEQ files that use it
          //  User warned to remove the move from those files first
          console.log('Move name change results in seq files')
          console.log(results)
          this.fixCombos(oldName, newName)
        }
        if (this.currentItem.$.length !== this.currentItemBeforeEdit.$.length) {
          this.seqFilesReplaceOptionsXML = {
            ...
            to: 'xxxxx' // DRY RUN - we just need a list of files with move
          }
    ...
    modifyFilesBeforeSave () {
      ...
      // get params for fixCombos from this.seqFilesReplaceOptionsXML
      try {
        const from = /["]+(.*)["]+/g.exec(this.seqFilesReplaceOptionsXML.from)[1]
        const to = /["]+(.*)["]+/g.exec(this.seqFilesReplaceOptionsXML.to)[1]
    ...
    saveEditedMove () {
      // if we pass preSaveEditMove, we get here.
      // remove under old name, insert under new
      let nInsertAt = 0
      if (this.currentItemBeforeEdit) { // editing existing move
