...
Moves: saved in moves.xml file">
        <b-col md="4" style="margin:0;" >
          ...
          <label for="timeMode" title="MODE: Beats must be created first; they are saved in the beats XML file (shortcut Alt-b)">Beats</label>
...
A Cambio gear is used to help automate meter changes - for example, a clave change in a song (see docs) (shortcut Alt-g).">Gears</label>
          <input type="radio" id="callsMode" value="callsMode" v-model="modePerRadios">
          <label for="callsMode" title="MODE: Your Call seqence is saved in SEQ file (shortcut Alt-c)">Call sequence</label>
        </b-col>
          ...
          class="mb-2">
            <b-row>
              <b-col>
                ...
                <ul>
                <li>Startup: {{currentComboHdr.startup}}</li>
  ...
  methods: {
    ...
    zapTheCalls (currentBeatIndex, shift, allRemaining = false) {
      ...
      this.$bvModal.msgBoxConfirm('Are you sure?', { title: guiString })
        ...
        .catch(e => {
          console.log('error msgBoxConfirm in Measures::zapTheCalls')
        })
    },
    setFocus () {
    ...
    promptAndHandle (title, msg, confirmButtonText, confirmHandler, closeHandler, useConfirmBtn = true) {
      const obj = {
        // useIcon: true,
        isShown: true,
        title: title,
        message: msg,
        type: 'warning',
        useConfirmBtn: useConfirmBtn,
        customConfirmBtnText: confirmButtonText,
        onConfirm: confirmHandler || this.nullSrtHandler,
        onClose: closeHandler || this.nullSrtHandler
      }
          ...
          if (this.beatAction) {
            if (this.beatAction === 'edit') {
              ...
              this.$bvModal.msgBoxConfirm('Are you sure?', options)
                ...
                .catch(e => {
                  console.log('error msgBoxConfirm in Measures::details')
                })
              this.beatAction = null // restore the default state
              ...
              this.$bvModal.msgBoxConfirm('Are you sure you want to delete this beat, and all following beats?', options)
                ...
                .catch(e => {
                  console.log('error msgBoxConfirm in Measures::details (2)')
                })
              this.beatAction = null // restore the default state
              return // all beatActions
              ...
              this.$bvModal.msgBoxConfirm('Are you sure you want to insert a beat between this beat and any previous one?', options)
                ...
                .catch(e => {
                  console.error('error msgBoxConfirm in Measures::details (3)')
                })
              this.beatAction = null // restore the default state
              return // all beatActions
            }
      ...
      if (this.mode === 'callsMode') {
        if (this.movesCombosOrAuto === 'autoMode') {
          ...
          for (let i = 0; i < allMoves.length; i++) {
            if (allMoves[i]) {
              const ind = this.editedMoves.binarySearchIndex(allMoves[i].move, m => m.$.nameSorted) // get the details need to save sequence
              const fullMove = this.editedMoves[ind]
              this.$emit('add-seq-move', allMoves[i].move) // Songs needs to add Howler for this to move dictionary
              this.$store.commit('ADD_THIS_SEQ_MOVE', { index: this.currentBeatIndex + offs, item: _cloneDeep(fullMove), maxBeat: this.lstTimesWork.length })
